...
LEGACY_OVERRIDE_DIR = AUTHORING_ROOT / "legacy-overrides"

CONTENT_DIR = AUTHORING_ROOT / "content"
...
PAPERS_OUTPUT_DIR = PUBLIC_ROOT / "papers"
TOPICS_OUTPUT_DIR = PUBLIC_ROOT / "topics"
REPLICATION_OUTPUT_DIR = PUBLIC_ROOT / "replication"
SOURCES_OUTPUT_DIR = PUBLIC_ROOT / "sources"
PAPER_READER_DATA_DIR = PUBLIC_ROOT / "paper-reader-data"
PAPER_ASSETS_DIR = PUBLIC_ROOT / "paper-assets"
GENERATED_DIR = AUTHORING_ROOT / "generated"
...
<head>
  ...
  {% if include_mathjax %}
  <script>
    window.__fitPaperMath = function __fitPaperMath() {
      const selectors = [
        '.paper-body .mathjax-block',
        '.paper-body .mathjax-env',
        '.paper-body .displaymath',
        '.paper-body table.equation',
        '.paper-body table.equation-star',
        '.paper-body div.equation',
        '.paper-body div.equation-star',
        '.paper-body .newtheorem .mathjax-inline',
        '.paper-body .tcolorbox .mathjax-inline'
      ];
      const requiredWidth = (container) => {
        let required = container.scrollWidth;
        const descendants = container.querySelectorAll(
          "mjx-container[display='true'], .mathjax-inline mjx-container, mjx-math, mjx-mtable, mjx-table, mjx-itable"
        );
      ...
      for (const container of containers) {
        if (container.closest('.sidenote, .marginnote')) {
          continue;
        }
        const baseFontSize =
          Number.parseFloat(container.dataset.baseFontSize || '') ||
          Number.parseFloat(window.getComputedStyle(container).fontSize);
        if (!Number.isFinite(baseFontSize) || baseFontSize <= 0) {
          continue;
        }
        container.dataset.baseFontSize = String(baseFontSize);
        container.style.fontSize = `${baseFontSize}px`;
        const widthOwner = container.classList.contains('mathjax-inline')
          ? container.closest('.newtheorem, .tcolorbox, p, li, dd')
          : container;
        const availableWidth = widthOwner ? widthOwner.clientWidth : container.clientWidth;
        if (!availableWidth) {
          continue;
        }
        let overflowWidth = requiredWidth(container);
        if (overflowWidth <= availableWidth + 2) {
          continue;
        }
        let scale = (availableWidth - 2) / overflowWidth;
        if (!Number.isFinite(scale) || scale >= 1) {
          continue;
        }
        for (let pass = 0; pass < 4; pass += 1) {
          container.style.fontSize = `${(baseFontSize * scale).toFixed(4)}px`;
          overflowWidth = requiredWidth(container);
          if (overflowWidth <= availableWidth + 2) {
            break;
          }
          scale *= (availableWidth - 2) / overflowWidth;
        }
      }
    };
      ...
      startup: {
        ready() {
          ...
          window.requestAnimationFrame(() => {
            if (window.__fitPaperMath) {
              window.__fitPaperMath();
            }
            window.dispatchEvent(new Event('paper-math-ready'));
          });
        }
      },
      chtml: {
  ...
  <script>
    (() => {
      ...
      window.addEventListener('paper-math-ready', scheduleFit);
...
def href_from(output_path: Path, target: str) -> str:
    ...
    target_path = PUBLIC_ROOT / rel_target
...
def refresh_paper_sources_with_opendataloader(papers: list[dict]) -> None:
    ...
    if not papers_to_refresh:
        return
    pdf_paths = [str(PUBLIC_ROOT / paper["pdf_url"]) for paper in papers_to_refresh]
...
def normalize_markdown_link_target(target: str) -> str:
    ...
    if not match:
        return stripped
    base, extension, suffix = match.groups()
    if extension:
        return stripped
    return f"{base}.html{suffix or ''}"
...
def render_inline(text: str) -> str:
    ...
    escaped = re.sub(
        r"\[([^\]]+)\]\(([^)]+)\)",
        lambda m: f'<a href="{html.escape(normalize_markdown_link_target(m.group(2)), quote=True)}">{m.group(1)}</a>',
        escaped,
    )
...
def parse_original_research_page() -> list[dict]:
    research_path = PUBLIC_ROOT / "Research.html"
...
def bootstrap_markdown_sources() -> None:
    ...
    if not any(PAPERS_CONTENT_DIR.glob("*.md")):
        ...
        for paper in papers:
            ...
            body = clean_pdf_to_markdown(PUBLIC_ROOT / paper["pdf_url"], paper["title"], paper["authors"])
...
def build_custom_latex_figure_overrides(
    ...
    if paper.get("slug") == "new-industrial-policy":
        lane_jpeg = LEGACY_OVERRIDE_DIR / "new-industrial-policy" / "Lane_QJE.jpg"
...
def build_latex_fragment(paper: dict) -> tuple[dict, dict]:
    slug = paper["slug"]
    latex_dir = AUTHORING_ROOT / paper["latex_dir"]
    ...
    try:
        with TemporaryDirectory(prefix=f"latex-{slug}-") as tmpdir:
            ...
            if "{AEA}" in tex_text and (AUTHORING_ROOT / "vendor" / "aea-latex-templates").exists():
                for template_file in (AUTHORING_ROOT / "vendor" / "aea-latex-templates").iterdir():
...
def render_research_hub(papers: list[dict], hubs: list[dict], queries: list[dict], topic_by_slug: dict) -> str:
    ...
    def render_paper_listing(paper: dict, index: int) -> str:
        ...
        if paper.get("slides_url"):
            assets.append(f'<a href="{href_from(PUBLIC_ROOT / "Research.html", paper["slides_url"])}">[slides]</a>')
        if paper.get("dashboard_url"):
            assets.append(f'<a href="{href_from(PUBLIC_ROOT / "Research.html", paper["dashboard_url"])}">[dashboard]</a>')
        if paper.get("online_appendix_url"):
            assets.append(f'<a href="{href_from(PUBLIC_ROOT / "Research.html", paper["online_appendix_url"])}">[online appendix]</a>')
        if paper.get("replication_slug"):
            assets.append(f'<a href="{href_from(PUBLIC_ROOT / "Research.html", "replication/" + paper["replication_slug"] + ".html")}">[replication files]</a>')
        elif paper.get("raw_replication_url"):
            assets.append(f'<a href="{href_from(PUBLIC_ROOT / "Research.html", paper["raw_replication_url"])}">[replication files]</a>')
...
def build_sitemap(urls: list[str]) -> None:
    ...
    write_if_changed(PUBLIC_ROOT / "sitemap.xml", sitemap)
...
def build_llms_files(hubs: list[dict], papers: list[dict], replications: list[dict]) -> None:
    ...
    llms += "\n"
    write_if_changed(PUBLIC_ROOT / "llms.txt", llms)
    ...
    full += "\n"
    write_if_changed(PUBLIC_ROOT / "llms-full.txt", full)
...
def refresh_legacy_replication_redirects() -> None:
    redirects = {
        PUBLIC_ROOT / "Replication_Page_Tariff_War.html": "replication/global-tariff-war-replication.html",
        PUBLIC_ROOT / "Replication_Page_Discrete_Trade.html": "replication/discrete-trade-replication.html",
    }
    for path, target in redirects.items():
        target_href = href_from(path, target)
        canonical_url = public_url(target)
...
<body>
  <p>Redirecting to <a href="{html.escape(target_href, quote=True)}">{html.escape(target_href)}</a>.</p>
</body>
</html>
"""
