...
Cargo.Config = {
		'CARGO_URL'					: (/cargocollective.com/.test(document.location.host)) ? document.location.host :  'cargocollective.com',
		'Print_Url'					: '',
		'Admin_Print_Url'			: '',
...
Cargo.Analytics = {
	...
	pageview: function(send_to_all_trackers, url) {
		if (typeof url !== 'undefined' && ! send_to_all_trackers) {
			ga(this.Config.name + '.send', 'pageview', {'page': url});
			return;
		}
		...
		if (typeof this.Config.custom_url !== 'undefined') {
			ga(this.Config.name + '.send', 'pageview', {'page': this.Config.custom_url + pathname});
		} else {
			ga(this.Config.name + '.send', 'pageview');
		}

		// Send data for any other trackers
		if (send_to_all_trackers) {
			ga(function() {
...
(function($) {
	...
	$.fn.FreshBox = function(options) {
		...
		this.LoadIMG = function($image) {
			...
			$(activeImage).css('cursor', function(){
				if ($.browser.mozilla) {
					return '-moz-zoom-out';
				} else if ($.browser.webkit) {
					return '-webkit-zoom-out';
...
$(document).ready(function() {
	...
	if ( (Cargo.Location.GetCargoUrl() || Cargo.Location.IsUserImageGallery()) && !Cargo.Location.IsInFrame()) {
		$.getJSON("http://"+Cargo.Config.get('CARGO_URL')+"/dispatch/cargosite/cargoToolset/"+Cargo.Location.GetDomainOrUrl()+"?callback=?", { is_following : Cargo.Location.IsFollowing() }, function(data) {
				...
				if( data.jdata.url ) {
					...
					if(Cargo.Location.IsCustomDomain()) {
						$.post(
							...
							{ token : data.jdata.token, url : data.jdata.url, muid : data.jdata.muid },
							function(dataObj) {
...
Cargo.Follow = {
	...
	Init : function() {
		...
		$(".follow_popup_ok").live('click', function() {
			Cargo.Follow.DoFollow($(this).attr('data-my_uid'), $(this).attr('data-their_uid'), $(this).attr('data-type'));
			return false;
		});

		$(".follow_popup_cancel").live('click', function() {
			Cargo.Follow.Hide();
			return false;
		});
	...
	DoFollow : function(my_uid, their_uid, type) {
		$("#following_loadspin").show();
		$.post(
			Cargo.Dispatcher.GetUrl("user/doFollow"), 
			{ my_uid:my_uid, their_uid:their_uid, type:type }, 
...
HiRes.Loader = {
	init: function() {
				...
				function disableHiRes() {
					// Cargo backend endpoint is unavailable on static hosts (e.g., GitHub Pages).
					// Keep front-end behavior without the server-side preference POST.
				}
